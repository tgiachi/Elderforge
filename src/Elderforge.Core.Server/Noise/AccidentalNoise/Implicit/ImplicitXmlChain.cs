...
namespace Elderforge.Core.Server.Noise.AccidentalNoise.Implicit;
...
public class ImplicitXmlChain : ImplicitModuleBase
{
    ...
    public static ImplicitXmlChain FromString(string text)
    {
        ...
        foreach (var xElement in xml.Descendants("module"))
        {
            ...
            {
                case "auto_correct":
                    chain.Modules.Add(name, AutoCorrectFromXElement(chain, xElement));
                    break;
                case "bias":
                    chain.Modules.Add(name, BiasFromXElement(chain, xElement));
                    break;
                case "blend":           throw new NotImplementedException("blend");
                case "bright_contrast": throw new NotImplementedException("bright_contrast");
                case "cache":
                    chain.Modules.Add(name, CacheFromXElement(chain, xElement));
                    break;
                case "ceiling": throw new NotImplementedException("ceiling");
                case "clamp":
                    chain.Modules.Add(name, ClampFromXElement(chain, xElement));
                    break;
                case "combiner":
                    chain.Modules.Add(name, CombinerFromXElement(chain, xElement));
                    break;
                case "constant":
                    chain.Modules.Add(name, ConstantFromXElement(chain, xElement));
                    break;
                case "cos":   throw new NotImplementedException("cos");
                case "floor": throw new NotImplementedException("floor");
                case "fractal":
                    chain.Modules.Add(name, FractalFromXElement(chain, xElement));
                    break;
                case "gain": throw new NotImplementedException("gain");
                case "gradient":
                    chain.Modules.Add(name, GradientFromXElement(chain, xElement));
                    break;
                ...
                case "scale_domain":
                    chain.Modules.Add(name, ScaleDomainFromXElement(chain, xElement));
                    break;
                case "scale_offset":
                    chain.Modules.Add(name, ScaleOffsetFromXElement(chain, xElement));
                    break;
                case "select":
                    chain.Modules.Add(name, SelectFromXElement(chain, xElement));
                    break;
                case "sin": throw new NotImplementedException("sin");
                case "sphere":
                    chain.Modules.Add(name, SphereFromXElement(chain, xElement));
                    break;
                case "tan":   throw new NotImplementedException("tan");
                case "tiers": throw new NotImplementedException("tiers");
                case "translate_domain":
                    chain.Modules.Add(name, TranslateDomainFromXElement(chain, xElement));
                    break;
    ...
    public static ImplicitConstant ConstantFromXElement(ImplicitXmlChain chain, XElement xElement)
    {
        ...
        return new ImplicitConstant(
            value == null ? 0.00 : double.Parse(value.Value, NumberStyles.Any, CultureInfo.InvariantCulture)
        );
    }

    public static ImplicitCombiner CombinerFromXElement(ImplicitXmlChain chain, XElement xElement)
    {
        var combinerTypeString = xElement.Attribute("combiner_type") != null
            ? xElement.Attribute("combiner_type").Value
            : string.Empty;
    ...
    public static ImplicitFractal FractalFromXElement(ImplicitXmlChain chain, XElement xElement)
    {
        ...
        var fractal = new ImplicitFractal(fractalType, basisType, interpolationType, 0, 1, 2);
